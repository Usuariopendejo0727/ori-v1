...
async def run_test():
    ...
    try:
        ...
        # -> Type the email into the email field (index 5) and the password into the password field (index 6), then click the login button (index 7).
        ...
        # -> Click on 'API y Modelo' in the main navigation to open the API configuration page (element index 887).
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        assert '/' in frame.url
        assert '/api-config' in frame.url
        await expect(frame.locator('text=Configuración guardada').first).to_be_visible(timeout=3000)
