...
async def run_test():
    ...
    try:
        ...
        # -> Type the login email into the email field (index 57). Then fill password (index 65) and click the login button (index 66).
        ...
        # -> Click on 'Knowledge Base' in the main navigation (index 166).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/nav/a[4]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Knowledge Base' link in the sidebar (use fresh index 939).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/nav/a[4]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Agregar URL' button in the Knowledge Base page (index 1608).
        frame = context.pages[-1]
        # Click element
        ...
        # Verify URL contains "/"
        assert "/" in frame.url
        # Verify the text "Agregar URL" is visible
        elem = frame.locator('xpath=/html/body/div[1]/div/main/div/div/div[3]').nth(0)
