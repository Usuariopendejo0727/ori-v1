...
async def run_test():
    ...
    try:
        ...
        # -> Type the email and password into the login form and click the 'Iniciar sesión' button to authenticate.
        ...
        # -> Navigate directly to /conversations (last-resort navigation) and then check for Conversations page elements (Conversations navigation and Leads tab).
        await page.goto("http://localhost:5173/conversations", wait_until="commit", timeout=10000)
        
        # -> Click the 'Leads' tab to switch to the Leads view and then verify the 'Exportar CSV' text is visible.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        frame = context.pages[-1]
        assert "/conversations" in frame.url
